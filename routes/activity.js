...
function registerActivityRoutes(app, deps) {
  ...
  app.get('/api/activity', (req, res) => {
    try {
      ...
      logger.info('API/activity', 'activity.list', { requestId: req.requestId, limit, offset, count: (result.items || []).length });
      res.json(result);
    } catch (e) {
      internalError(res, e);
    }
  ...
  app.get('/api/export/markdown', (req, res) => {
    try {
      ...
      logger.info('API/activity', 'activity.export_markdown', { requestId: req.requestId, days, bytes: Buffer.byteLength(out, 'utf-8') });
